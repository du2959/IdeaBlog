$(document).ready(function() {
    ...
    $("#sysLogManage").click(function () {
        ...
        $("#feedbackManage").removeClass("active");
    ...
    $("#feedbackManage").click(function () {
        ...
        $("#table").bootstrapTable('destroy');
        $("#table").bootstrapTable({ // 对应table标签的id
            url: "showAllFeedbacks.do",                           //请求后台的URL（*）
            method: 'post',                     //请求方式（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
            ...
            columns: [
                ...
                },
                {
                    ...
                    formatter: function (value, row, index) { // 单元格格式化函数
                        var text = '-';
                        if (value == 1) {
                            text = "<button class='btn btn-warning' onclick='switchFeedbackStatus(" + row.id + "," + row.status + ");'>待处理</button>";
                        } else if (value == 2) {
                            text = "<button class='btn btn-success' onclick='switchFeedbackStatus(" + row.id + "," + row.status + ");'>已处理</button>";
                        }
                        return text;
                    }
                },
                {
                    ...
                    valign: 'middle',
                    formatter: function (value, row, index) {
                        return "<button class='btn btn-danger' onclick='deleteFeedback(" + row.id + ");'>删除</button>";
                    }
                }
            ],
            onLoadSuccess: function () {  //加载成功时执行
                console.info("反馈列表加载成功");
            },
            onLoadError: function () {  //加载失败时执行
                console.info("加载数据失败");
            }
...
function switchFeedbackStatus(feedbackId, status){
    if(confirm("确认切换反馈处理状态？")){
        $.ajax({
            type:"post",
            url:"switchFeedbackStatus.do?feedbackId=" + feedbackId + "&status=" +(status == 1 ? 2 : 1),
            async:true,
            success:function(data){
                console.log(data);
                $("#table").bootstrapTable('refresh', {url:'showAllFeedbacks.do'});
            },
            error:function(){
                console.log("error");
            }
...
function deleteFeedback(feedbackId){
    if(confirm("确认删除该反馈？（不可恢复）ID：" + feedbackId)){
        $.ajax({
            type:"post",
            url:"deleteFeedback.do?feedbackId=" + feedbackId,
            async:true,
            success:function(data){
                console.log(data);
                $("#table").bootstrapTable('refresh', {url:'showAllFeedbacks.do'});
